...
class YouTubePlaylistGenerator:
    def __init__(self, cookies_file='cookies.txt'):
        ...
        for d in [self.channels_dir, self.logos_dir]:
    ...
    def get_stream_info(self, url):
        ...
        try:
            with yt_dlp.YoutubeDL(ydl_opts) as ydl:
                ...
                is_live = live_status in ['is_live', 'is_upcoming', 'live'] or \
                          info.get('is_live') or info.get('was_live')

                formats = info.get('formats', [])

                # Get all video formats with URLs
                video_formats = [
                    f for f in formats
                    if f.get('url') and f.get('vcodec', '') != 'none'
                    and not f.get('url', '').startswith('https://www.youtube.com/watch')
                ]

                if not video_formats:
                    # Try getting the direct URL
                    direct_url = info.get('url', '')
                    if direct_url:
                        video_formats = [{'url': direct_url, 'height': 0, 'quality_tag': 'Auto'}]

                if not video_formats:
                    print(f"  ⚫ No stream URL found (status: {live_status})")
                    return {
                        'status': 'offline',
                        'video_id': video_id,
                ...
                status = 'live' if is_live else 'active'
                print(f"  ✅ {status.upper()} - qualities: {list(streams.keys())} - {clean_name}")
    ...
    def generate_playlists(self, channels_data):
        ...
        header = [
            ...
            "#EXT-X-VERSION:3",
            f"# Generated: {now_utc}",
            f"# Total channels: {len(channels_data)}",
            ""
        ]
        ...
        for ch in channels_data:
            ...
            if ch.get('status') == 'live':
                ...
                main_stream = ch.get('streams', {}).get('hd') or \
                              next(iter(ch.get('streams', {}).values()), None)
                mobile_stream = ch.get('streams', {}).get('mobile')

                if main_stream:
                    qtag = main_stream.get('quality_tag', 'Auto')
                    main_lines += [
                        f'#EXTINF:-1 tvg-id="{channel_id}" tvg-name="{name}" group-title="Live",{name} [{qtag}] 🔴',
                        main_stream['url'], ""
                    ]
                    hd_lines += [
                        f'#EXTINF:-1 tvg-id="{channel_id}" tvg-name="{name}" group-title="HD",{name} [HD]',
                        main_stream['url'], ""
                    ]

                if mobile_stream:
                    mobile_lines += [
                        f'#EXTINF:-1 tvg-id="{channel_id}" tvg-name="{name}" group-title="Mobile",{name} [Mobile]',
                        mobile_stream['url'], ""
                    ]
                elif main_stream:
                    mobile_lines += [
                ...
                for lines in [main_lines, hd_lines, mobile_lines]:
                    lines += [
                        f'#EXTINF:-1 tvg-id="{channel_id}" tvg-name="{name}" group-title="Offline",{name} [Offline]',
                        fallback, ""
                    ]
...
def main():
    ...
    if not lines:
        print("⚠️ No URLs in streams.txt")
        return
